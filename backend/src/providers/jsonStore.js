...
function validateTaskPayload(payload, { partial = false } = {}) {
  ...
  if (!partial || Object.prototype.hasOwnProperty.call(payload, 'q')) {
    if (!allowedQuadrants.has(payload.q)) errors.push('q must be one of Q1, Q2, Q3, Q4');
  }
  if (!partial || Object.prototype.hasOwnProperty.call(payload, 'cat')) {
    if (!allowedCategories.has(payload.cat)) errors.push('cat must be a valid category');
  }
  if (!partial || Object.prototype.hasOwnProperty.call(payload, 'energy')) {
    if (!allowedEnergy.has(payload.energy)) errors.push('energy must be high, medium, or low');
  }
...
async function getTasks(filters = {}) {
  ...
  if (filters.date) tasks = tasks.filter((task) => task.date === normalizeDate(filters.date));
  if (filters.from) tasks = tasks.filter((task) => task.date >= normalizeDate(filters.from));
  if (filters.to) tasks = tasks.filter((task) => task.date <= normalizeDate(filters.to));
  if (filters.done === true || filters.done === false) tasks = tasks.filter((task) => task.done === filters.done);
  if (filters.q) tasks = tasks.filter((task) => task.q === filters.q);
  if (filters.cat) tasks = tasks.filter((task) => task.cat === filters.cat);

  return { store, tasks: sortTasks(tasks) };
}
...
async function createTask(payload) {
  ...
  if (errors.length > 0) throw createValidationError(errors);
